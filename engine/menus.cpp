...
char* loadGame(char *pname)
{
    SQLiteConnector connector = SQLiteConnector();
    savedGame legame = savedGame();
    legame = connector.getGame(pname);
    return legame.getMap();
    //game::playerobj->s_hp = (char)legame.getHealth();
}

char* returnMapa(char* a)
{
    char* asda;
    sprintf(asda,"loadGame %s",a);
    return asda;
}

void listGames()
{
    //char debugvariable[255];
    SQLiteConnector connector = SQLiteConnector();
    vector<savedGame> thegames = vector<savedGame>();
    savedGame sv;
    string c;
    thegames = connector.listGames();
    string mapa;
    //bool compare = false;
    //string compares;
    for(int i=0 ;i < thegames.length();i++)
    {
        ...
        //{
            sprintf(mapa,"sp");
            sprintf(c,"%s %s",mapa,loadGame(sv.getName()));
            guibutton(sv.getName(),c, "cube");
            //compare = true;
            //sprintf(compares,sv.getName());
            guibar();
        /*}
        else
            {
                if(stricmp(compares,sv.getName())== 0)
                {
                    compare = true;
                }
                else
...
void listGamesD()
{
    ...
    for(int i=0 ;i < thegames.length();i++)
    {
        ...
        //if(compare == false)
        //{
            sprintf(mapa,"deleteGameX %s",sv.getName());
            guibutton(sv.getName(),mapa, "cube");
            guibar();
       /* }
        else
            {
...
void deleteGame(char *pname)
{
    SQLiteConnector connector = SQLiteConnector();
    connector.deleteGame(pname);
}

void deleteGameX(char *pname)
{
    deleteGame(pname);
    showgui("save");
}
...
void showAchievements()
{
    //char debugvariable[255];
    SQLiteConnector connector = SQLiteConnector();
    vector<Achievement> thegames = vector<Achievement>();
    Achievement sv;
    thegames = connector.listAchievements();
    string mapa;
    //string losLogros;
    //bool compare = false;
    string compares;
    for(int i=0 ;i < thegames.length();i++)
    {
        ...
        //if(compare == false)
        //{
            char nombre[1024], desc[1024], status[1024];
            //sprintf(losLogros, "guitext \"Name: %s \" \"cube\" \n guitext Description: %s \" \" \" \n guitext Status: %s \" \" \" \n guibar",sv.getName(),sv.getDescription(),sv.getStatus());
            sprintf(nombre,"Name: %s ",sv.getName());
            sprintf(desc,"Description: %s ",sv.getDescription());
            sprintf(status,"Status: %s ",sv.getStatus());
            guitext(nombre ,"");
            guitext(status ,"");
            guitext(desc ,"");
            guibar();
            //compare = true;
            //sprintf(compares,sv.getId());
            //newgui("Achievements",losLogros);
        //}
        /*else
            {
                if(stricmp(compares,sv.getId())== 0)
                {
                    compare = true;
                }
                else
...
void setAchievement(char *pname)
{
    if(*pname)
        {
        char unl[1024];
        Achievement legame = Achievement();
        SQLiteConnector connector = SQLiteConnector();
        legame = connector.getAch(pname);
        sprintf(unl,"Achievement: %s UNLOCKED!",legame.getName());
        conoutf(unl);
        legame.setStatus("UNLOCKED");
        connector.createAchievement(legame);
        }
}


COMMAND(deleteGame, "s");
COMMAND(deleteGameX, "s");
COMMAND(loadGame, "s");
COMMAND(listGames, "");
COMMAND(listGamesD, "");
COMMAND(showAchievements,"");
COMMAND(setAchievement,"s");
